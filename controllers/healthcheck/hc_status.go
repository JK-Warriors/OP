...
//上班巡检
type ManageOndutyController struct {
	controllers.BaseController
}

func (this *ManageOndutyController) Get() {
	//权限检测
...
//下班巡检
type ManageOffdutyController struct {
	controllers.BaseController
}

func (this *ManageOffdutyController) Get() {
	//权限检测
	if !strings.Contains(this.GetSession("userPermission").(string), "hc-config-manage") {
		this.Abort("401")
	}
...
//下班巡检
type ManageManualController struct {
	controllers.BaseController
}

func (this *ManageManualController) Get() {
	//权限检测
	if !strings.Contains(this.GetSession("userPermission").(string), "hc-config-manage") {
		this.Abort("401")
	}
