...
func SwitchMirror(op_id int64, bs_id int, dsn_p string, dsn_s string, db_name string) int {
	...
	Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "获取镜像角色成功")
	...
	Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "获取镜像状态成功")
	utils.LogDebugf("The current mirror state is: %d", state)

	if role == 1 {
		Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "验证镜像角色成功")
		...
		if state == 4 {
			...
			if err != nil {
				...
				Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "主库已经成功切换成高安全模式")
			}
			...
			if role == 2{
				Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "主体已经成功切换成镜像")
				...
				Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "镜像已经成功切换成高性能模式")
				utils.LogDebug("SET SAFETY OFF successfully.")
                
                result=0
			}else{
				Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "主体切换成镜像失败")
				utils.LogDebug("SET PARTNER FAILOVER failed.")
                result=-1
			}
		}else{
			Update_OP_Reason(op_id, "验证镜像状态失败，当前镜像状态不是'已同步'")
			Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "验证镜像状态失败，当前镜像状态不是'已同步'")
			utils.LogDebug("Check mirror state failed!")
			result = -1
		}
	}else {
		Update_OP_Reason(op_id, "验证镜像角色失败，当前镜像不是主体，不能切换镜像")
		Log_OP_Process(op_id, bs_id, 1, "SWITCHOVER", "验证镜像角色失败，当前镜像不是主体，不能切换镜像")
