...
func GatherDbStatus(mysql *xorm.Engine, db_id int){
	...
	Check_Item_Status(mysql, db_id, "repl", "repl", mrpstatus)
	Check_Item_Status(mysql, db_id, "repl_delay", "repl_delay", dgdelay)
	
	max_rate := GetMaxTablespaceRate(mysql, db_id)
	Check_Item_Status(mysql, db_id, "tablespace", "tablespace", max_rate)
}
...
func GetColumnValue(mysql *xorm.Engine, column string, db_id int) string{
	...
	sql := `select ` + column + ` from pms_oracle_status where db_id = ?`
...
func GetMaxTablespaceRate(mysql *xorm.Engine, db_id int) string{
	var max_value string = "-1"

	sql := `select max(max_rate) from pms_oracle_tablespace where db_id = ?`

	_, err := mysql.SQL(sql, db_id).Get(&max_value)
	if err != nil {
		log.Printf("%s: %s", sql, err.Error())
	}
