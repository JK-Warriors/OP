...
func GatherDbStatus(mysql *xorm.Engine, db_id int){
	...
	session_total := GetColumnValue(mysql, "sessions", db_id)
	Check_Item_Status(mysql, db_id, "session_total", "sessions", session_total)

	session_actives := GetColumnValue(mysql, "processes_running", db_id)
	Check_Item_Status(mysql, db_id, "session_actives", "actives", session_actives)

	session_waits := GetColumnValue(mysql, "processes_waits", db_id)
	...
	Check_Item_Status(mysql, db_id, "repl", "repl", "-1")
	Check_Item_Status(mysql, db_id, "repl_delay", "repl_delay", "-1")
...
func GetColumnValue(mysql *xorm.Engine, column string, db_id int) string{
	...
	sql := `select ` + column + ` from pms_mssql_status where db_id = ?`
