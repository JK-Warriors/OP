...
func AlertBasicInfo(mysql *xorm.Engine, db_id int){
	...
	log.Printf("Alert replication status: %s", repl_status)
	ExecTriggers(mysql, db_id, "repl_status", "", repl_status)

	repl_delay := GetReplicationDelay(mysql, db_id)
	log.Printf("Alert replication delay: %s", repl_delay)
	ExecTriggers(mysql, db_id, "repl_delay", "", repl_delay)
}
...
func GetConnect(mysql *xorm.Engine, db_id int) string{
	...
	sql := `select connect from pms_mysql_status where db_id = ?`
...
func GetReplicationStatus(mysql *xorm.Engine, db_id int) string{
	var repl_status string = "-1"

	sql := `select repl from pms_asset_status where asset_id = ?`

	_, err := mysql.SQL(sql, db_id).Get(&repl_status)
	if err != nil {
		log.Printf("%s: %s", sql, err.Error())
	}
...
func GetReplicationDelay(mysql *xorm.Engine, db_id int) string{
	var repl_delay string = "-1"

	sql := `select repl_delay from pms_asset_status where asset_id = ?`

	_, err := mysql.SQL(sql, db_id).Get(&repl_delay)
	if err != nil {
		log.Printf("%s: %s", sql, err.Error())
	}
