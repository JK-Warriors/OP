      ...
      <div class="row">
        <div class="col-sm-12">
          <!-- 主体内容 开始 -->
              ...
              <div class="search-form">
                <div class="form-inline">
                  <div class="form-group">
                    <form action="/oracle/asm/manage" method="get">
                    ...
                    <a href="/oracle/asm/manage" class="btn btn-default" type="submit"> <i class="fa fa-reset"></i> 重置 </a>
                    </form>
                  </div>
                </div>
              </div>
            ...
            <section class="panel">
              <header class="panel-heading"> 磁盘组列表 / 总数：{{.CountDiskgroup}}
                ...
                <section id="unseen">
                  <form id="user-form-list">
                    <table class="table table-bordered table-striped table-condensed">
                      ...
                      <tbody>
                      {{range $k,$v := .diskgroups}}
                        <tr>
                          <td>{{$v.Alias}}</td>
                          <td>{{$v.Host}}</td>
                          <td>{{$v.Diskgroup_Name}}</td>
                          <td>{{$v.State}}</td>
                          <td>{{$v.Type}}</td>
                          <td>{{$v.Total_Mb}}</td>
                          <td>{{$v.Free_Mb}}</td>
